...
///////////////////////////////////////////////////////////////////
void CFft::SetFFTAve(int32_t ave)
{
  if (m_AveSize != ave)
  {
    if (ave > 0)
...
//FFT initialization and parameter setup function
///////////////////////////////////////////////////////////////////
void CFft::SetFFTParams(int32_t size, bool invert, TYPEREAL dBCompensation, TYPEREAL SampleFreq)
{
  int32_t i;
  if (size == 0)
    return;
  ...
  if (m_LastFFTSize != m_FFTSize)
  {
    ...
    m_pWorkArea = new int32_t[(int32_t)MSQRT((TYPEREAL)m_FFTSize) + 2];
    ...
    m_pTranslateTbl = new int32_t[m_FFTSize];
    ...
    for (i = 0; i < m_FFTSize; i++) //Nuttall
      m_pWindowTbl[i] = WindowGain * (0.355768 - 0.487396 * MCOS((K_2PI * i) / (m_FFTSize - 1)) +
...
//////////////////////////////////////////////////////////////////////
int32_t CFft::PutInDisplayFFT(int32_t n, TYPECPX* InBuf)
{
  int32_t i;
  m_Overload = false;
...
//			must be >= to K_MINDB
//////////////////////////////////////////////////////////////////////
bool CFft::GetScreenIntegerFFTData(int32_t MaxHeight,
                                   int32_t MaxWidth,
                                   TYPEREAL MaxdB,
                                   TYPEREAL MindB,
                                   int32_t StartFreq,
                                   ...
                                   int32_t* OutBuf)
{
  ...
  if ((m_StartFreq != StartFreq) || (m_StopFreq != StopFreq) || (m_PlotWidth != MaxWidth))
  { //if something has changed need to redo translate table
    m_StartFreq = StartFreq;
    m_StopFreq = StopFreq;
    m_PlotWidth = MaxWidth;
    maxbin = m_FFTSize - 1;
    m_BinMin = (int32_t)((TYPEREAL)StartFreq * (TYPEREAL)m_FFTSize / m_SampleFreq);
    m_BinMin += (m_FFTSize / 2);
    m_BinMax = (int32_t)((TYPEREAL)StopFreq * (TYPEREAL)m_FFTSize / m_SampleFreq);
  ...
  if ((m_BinMax - m_BinMin) > m_PlotWidth)
  {
    ...
    {
      if (m_Invert)
        y = (int32_t)((TYPEREAL)MaxHeight * dBGainFactor * (m_pFFTAveBuf[(m - i)] - dBmaxOffset));
      else
        y = (int32_t)((TYPEREAL)MaxHeight * dBGainFactor * (m_pFFTAveBuf[i] - dBmaxOffset));
      if (y < 0)
        y = 0;
      if (y > MaxHeight)
        y = MaxHeight;
...
///////////////////////////////////////////////////////////////////
void CFft::rftfsub(int32_t n, TYPEREAL* a, int32_t nc, TYPEREAL* c)
{
  int32_t j, k, kk, ks, m;
...
///////////////////////////////////////////////////////////////////
void CFft::CpxFFT(int32_t n, TYPEREAL* a, TYPEREAL* w)
{
...
///////////////////////////////////////////////////////////////////
void CFft::makewt(int32_t nw, int32_t* ip, TYPEREAL* w)
{
  int32_t j, nwh;
  TYPEREAL delta, x, y;
...
///////////////////////////////////////////////////////////////////
void CFft::makect(int32_t nc, int32_t* ip, TYPEREAL* c)
{
  int32_t j, nch;
  TYPEREAL delta;
...
/* -------- child routines -------- */
///////////////////////////////////////////////////////////////////
void CFft::bitrv2(int32_t n, int32_t* ip, TYPEREAL* a)
{
...
///////////////////////////////////////////////////////////////////
void CFft::cftfsub(int32_t n, TYPEREAL* a, TYPEREAL* w)
{
  int32_t j, j1, j2, j3, l;
  TYPEREAL x0r, x0i, x1r, x1i, x2r, x2i, x3r, x3i;
...
///////////////////////////////////////////////////////////////////
void CFft::cft1st(int32_t n, TYPEREAL* a, TYPEREAL* w)
{
  int32_t j, k1, k2;
...
///////////////////////////////////////////////////////////////////
void CFft::cftmdl(int32_t n, int32_t l, TYPEREAL* a, TYPEREAL* w)
{
